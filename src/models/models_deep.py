...
class Simple3DCNN_PatchSensitivity(nn.Module):
    """
    Variant of Simple3DCNN with spectral-only pooling.
    Used for patch size sensitivity experiment only.
    Does not affect main pipeline results.
    """
    def __init__(self, num_classes: int):
        super().__init__()
        self.conv1 = nn.Conv3d(
            in_channels=1, out_channels=16,
            kernel_size=(3, 3, 7), padding=(1, 1, 3),
        )
        self.bn1 = nn.BatchNorm3d(16)
        self.pool1 = nn.MaxPool3d(kernel_size=(2, 1, 1))  # spectral only
        ...
        self.bn2 = nn.BatchNorm3d(32)
        self.pool2 = nn.MaxPool3d(kernel_size=(2, 1, 1))  # spectral only
...
def train_3dcnn_patch_experiment(patches_train, y_train,
                                  ...
                                  device=None):
    """
    Uses Simple3DCNN_PatchSensitivity — spectral-only pooling.
    Safe for any patch size. Does not touch main pipeline.
    """
    if device is None:
        device = "cuda" if torch.cuda.is_available() else "cpu"
